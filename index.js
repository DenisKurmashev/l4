 ...
 */
const hash = _str => {
  const str = bin(_str); // Бинарное представление строки
  const emptyBits = N - (str.length % N); // Количество битов для добавления
  let input = str;
  let result = "0"; // Результат функции хэширования

  // 1
  // Разбиение на блоки, заполнение пустых мест 0
  // Добавление нулей в пустые биты
  for (let i = 0; i < emptyBits; i++) {
    input += "0";
  }
  ...
  // Массив блоков в бинарном виде входной последовательности
  const blocks = [];

  // Разбитие
  for (let i = 0; i < input.length; i += N) {
    blocks.push(input.substring(i, i + N));
  }

  // 2
  // Добавление последним блоком (X[T + 1]) длинны сообщение
  blocks.push(bin(input.length.toString()));

  // 3
  // Генерация хеша на основе результатов шаговой функции
  // encode -  шаговая функция хэширования
  for (const block of blocks) {
    result = encode(bin(result) + block, block);
  }

  return result;
};

const mess = "0DAE3BF4ECCAD161";
const res = hash(mess);
